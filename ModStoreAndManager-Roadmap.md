...
# Desktop-Implementierungstracking

## GAD-001 – Game-Adapter + Capability Registry

- **Bereich:** Architektur / P0
- **Priorität:** P0
- **Status:** `IN_ARBEIT`
- **Abhängigkeiten:** Keine; Grundlage für Deployment, Profile, Launch und Kompatibilität.
- **Betroffene Komponenten:** `Services/GameAdapters/`, `ModDependencyService`, `ModsFolderSyncService`, DI-Registry.
- **Betroffene APIs:** Keine Webapp-API-Änderung; `SteamConstants.DataCenterAppId` bleibt unverändert.
- **Umgesetzt:** Adapter-ID, AppID, Detection, OS-abhängige Executable-/Unity-Pfade, Capability Registry, Compatibility- und Install-/Uninstall-/Launch-Pläne, Path-Traversal-Schutz.
- **Akzeptanzkriterien:**
  - [x] Adapter wird über DI registriert und von bestehenden Dependency-/Workshop-Pfaden genutzt.
  - [x] Detection, Save-/Mod-/Plugin-/Workshop-Pfade und Compatibility sind getestet.
  - [x] Install-/Uninstall-/Launch-Pläne validieren Pfade und fehlende Quellen.
  - [ ] Tatsächliche transaktionale Deploy-/Uninstall-Ausführung ist implementiert.
  - [ ] Adapter-Contract-Tests decken die ausgeführten Operationen auf Windows, Linux und macOS ab.
- **Tests:** `GameAdapterTests` und gesamte Desktop-Test-Suite; 16 Tests bestanden mit `DOTNET_ROLL_FORWARD=Major`.
- **Risiken:** Data-Center-Executable-Namen und macOS/Linux-Layout müssen mit realen Installationen verifiziert werden.
- **Ergebnis:** Adaptergrundlage implementiert; wegen fehlender Ausführungs-/Rollback-Engine noch nicht `ERLEDIGT`.

## GAP-001 – Desktop/Webapp-Gap-Matrix

- **Bereich:** Analyse / P0
- **Priorität:** P0
- **Status:** `ERLEDIGT`
- **Abhängigkeiten:** Lesen von `current_features.md`, `functions.md`, `api-compatibility.md` und tatsächlichem Quellcode.
- **Betroffene Komponenten:** Desktop `Core`/Avalonia, Webapp `src/app/api`, Dokumentation.
- **Betroffene APIs:** Auth-Verträge geprüft; keine Änderung.
- **Akzeptanzkriterien:** Gap-Matrix mit Statuswerten, Evidenz und nächsten Schritten vorhanden.
- **Tests:** Dokumentationsprüfung, Desktop-Build/Test und Webapp-Routeninventar.
- **Risiken:** Externe Infrastruktur/DNS und produktive Webapp-Konfiguration sind nicht lokal verifiziert.
- **Ergebnis:** `functions.md`, `current_features.md`, `api-compatibility.md` und `docs/codebase/*` aktualisiert.

## UX-001 – Dialoge, Seitenlayout und Modstore-Ansichten

- **Bereich:** Desktop UX / Modstore
- **Priorität:** P1
- **Status:** `REVIEW`
- **Abhängigkeiten:** Bestehende Steam-Browse-, Subscribe- und Workshop-Sync-Dienste.
- **Betroffene Komponenten:** `DialogService`, `TerminalCore.axaml`, `SettingsPage`, `NewProjectPage`, `ModManagerPage`, `MainWindow`.
- **Akzeptanzkriterien:**
  - [x] Popups teilen MainWindow-Titelleiste, Farben und Aktionsbutton-Stile.
  - [x] Lange Seiten und Dialogtexte besitzen vertikales Scrollen.
  - [x] Modstore zeigt Installationsstatus und Preview-Bilder.
  - [x] Listen- und Galerieansicht sind umschaltbar.
  - [x] Mehrere Store-Items können ausgewählt und über den vorhandenen Subscribe-/Sync-Weg installiert werden.
  - [ ] Echte Desktop-/Webapp-Katalogparität und serverseitige Mass-Install-Projektauflösung bleiben offen.
- **Tests:** Release-Build erfolgreich; 16 Desktop-Tests bestanden; UI-Manuelltest mit Steam-Konto noch erforderlich.
- **Risiken:** Steam-IsInstalled/NeedsUpdate hängt vom Steam-UGC-Zustand ab; Bilder benötigen erreichbare Preview-URLs.
- **Ergebnis:** Implementiert und zur manuellen UI-/Steam-Verifikation bereit.
